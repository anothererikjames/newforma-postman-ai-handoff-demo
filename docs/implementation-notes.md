...
| the Postman CLI run | **Real (when configured)** | `npm run test:postman` runs the generated collection against the locally running API. The tests pass because the API and the templates agree on behavior — that contract is the point of the demo. |
...
- **Dependencies are deliberately minimal.** Runtime: `express` only. Validation is hand-rolled (`src/validation/schemas.ts`) to keep the repo skimmable. Dev: `typescript`, `tsx`, `the Postman CLI`, types.
